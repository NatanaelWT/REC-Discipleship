...
if ($page === 'msk_classes') {
    ...
    render_condition_alerts([
        ...
        ['when' => isset($_GET['converted']), 'tone' => 'success', 'message' => 'Peserta luar jemaat yang menyelesaikan 12 sesi otomatis ditambahkan ke data jemaat.'],
    ]);
    $error = trim((string) ($_GET['error'] ?? ''));
    render_mapped_error_alert($error, [
        'invalid_msk_source' => 'Jenis peserta tidak valid. Pilih dari daftar jemaat atau luar jemaat.',
        'missing_msk_member' => 'Pilih jemaat jika jenis peserta adalah dari daftar jemaat.',
        'duplicate_msk_member' => 'Jemaat tersebut sudah terdaftar di kelas MSK dan tidak bisa dipilih lagi.',
        'missing_msk_name' => 'Nama peserta luar jemaat wajib diisi.',
    ...
    $renderMskParticipantForm = function (array $participant, string $batchMonth, string $closeActionAttr = ''): string {
        ...
        ob_start();
        echo "<form method=\"post\" enctype=\"multipart/form-data\" class=\"form-grid msk-participant-form\" data-msk-form>\n";
        ...
        if ($closeActionAttr === '') {
            echo "    <a class=\"btn ghost\" href=\"?page=msk_classes&batch_month=" . h($batchMonth) . "\">Batal</a>\n";
        }
        echo "  </div>\n";
        echo "</form>\n";
    ...
    $appendMskViewTemplate = function (array $participant) use (&$mskModalTemplates, $batchMonthFilterParam, $centralReadOnly): void {
        ...
        if (!$centralReadOnly) {
            $templateData['edit_href'] = '?page=msk_classes&edit=' . rawurlencode($viewParticipantId) . '&batch_month=' . rawurlencode($batchMonthFilterParam);
        }
    ...
    if (!$centralReadOnly) {
        ...
        echo "      <form method=\"post\" enctype=\"multipart/form-data\" class=\"msk-import-inline-form\">\n";
        ...
        echo "      </form>\n";
        echo "      <form method=\"post\" class=\"msk-export-inline-form\">\n";
    ...
    if (count($batchMonthOptions) > 0) {
        echo "      <form method=\"get\" class=\"form-row cash-filter-form\">\n";
        echo "      <input type=\"hidden\" name=\"page\" value=\"msk_classes\">\n";
    ...
    foreach ($participantsFilteredByBatch as $participant) {
        ...
        if (!$centralReadOnly) {
            ...
            echo "<form method=\"post\" class=\"inline\" onsubmit=\"return confirm('" . h($toggleConfirm) . "');\">";
