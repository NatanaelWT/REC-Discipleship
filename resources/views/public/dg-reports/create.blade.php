...
@section('content')
    ...
    <section class="card public-dg-report-card">
      ...
      <form method="post" action="{{ route('public.dg.report.store', ['branch' => $publicBranch]) }}" enctype="multipart/form-data" class="form-grid dg-public-report-form" data-dg-public-form data-dg-hard-disabled="{{ $formDisabled ? '1' : '0' }}">
        ...
        <label class="dg-question dg-panel dg-section-photo"><span class="question-label">Foto Pertemuan</span><span class="dg-upload-field" data-dg-upload-field><span class="dg-upload-copy"><span class="dg-upload-badge">Pilih Foto</span><span class="dg-upload-meta" data-dg-upload-label>Belum ada file dipilih</span></span><span class="dg-upload-hint">JPG, PNG, atau WEBP. Original tetap disimpan; versi hemat data dibuat di perangkat ini.</span><span class="dg-upload-meta" data-image-variant-status></span><input type="file" name="meeting_photos[]" accept="image/jpeg,image/png,image/webp" data-dg-photo-input data-dg-requires-group data-client-image-variants data-web-variant-name="meeting_photo_web_variants[]" data-thumbnail-name="meeting_photo_thumbnails[]" @disabled(! $dgFormUnlocked) multiple></span></label>
