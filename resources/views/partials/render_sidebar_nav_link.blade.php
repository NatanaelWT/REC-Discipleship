...
function render_sidebar_nav_href(string $href): string {
    ...
    if ($page === '' || !class_exists(\App\Services\Legacy\LegacyRouteMap::class)) {
        return $href;
    }

    if (!\App\Services\Legacy\LegacyRouteMap::hasPage($page)) {
        return $href;
    }

    return \App\Services\Legacy\LegacyRouteMap::pageUrl($page, $params);
}
