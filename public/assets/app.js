(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const memberViewModal = document.querySelector('[data-member-view-modal]');
    if (memberViewModal) {
      const titleEl = memberViewModal.querySelector('[data-member-view-title]');
      const bodyEl = memberViewModal.querySelector('[data-member-view-body]');
      const editLinkEl = memberViewModal.querySelector('[data-member-view-edit-link]');
      const closeButtons = memberViewModal.querySelectorAll('[data-member-view-close]');
      const templateMap = new Map();

      document.querySelectorAll('template[data-member-view-template]').forEach((templateEl) => {
        const memberId = templateEl.getAttribute('data-member-view-template') || '';
        if (memberId) {
          templateMap.set(memberId, templateEl);
        }
      });

      const openMemberView = (memberId) => {
        const key = String(memberId || '').trim();
        if (!key || !templateMap.has(key)) {
          return;
        }
        const templateEl = templateMap.get(key);
        const modalTitle = templateEl?.getAttribute('data-member-view-template-title') || 'Detail Jemaat';
        const editHref = templateEl?.getAttribute('data-member-view-template-edit') || '?page=members';
        ...
        memberViewModal.classList.add('is-open');
        memberViewModal.setAttribute('aria-hidden', 'false');
        document.body.classList.add('modal-open');
      };

      const closeMemberView = () => {
        memberViewModal.classList.remove('is-open');
        memberViewModal.setAttribute('aria-hidden', 'true');
        syncBodyModalState();
      };

      document.addEventListener('click', function (event) {
        const trigger = event.target.closest('[data-member-view-open]');
        if (!trigger) {
          return;
        }
        event.preventDefault();
        openMemberView(trigger.getAttribute('data-member-view-open') || '');
      });
      ...
      memberViewModal.addEventListener('click', function (event) {
        if (event.target === memberViewModal) {
          closeMemberView();
        }
      });

      document.addEventListener('keydown', function (event) {
        if (document.querySelector('[data-file-preview-modal].is-open')) {
          return;
        }
        if (event.key === 'Escape' && memberViewModal.classList.contains('is-open')) {
          closeMemberView();
        }
      });

      const autoOpenId = memberViewModal.getAttribute('data-member-view-auto-open') || '';
      if (autoOpenId) {
        openMemberView(autoOpenId);
      }
    }

    const memberEditModal = document.querySelector('[data-member-edit-modal]');
    if (memberEditModal) {
      const titleEl = memberEditModal.querySelector('[data-member-edit-title]');
      const bodyEl = memberEditModal.querySelector('[data-member-edit-body]');
      const templateMap = new Map();

      document.querySelectorAll('template[data-member-edit-template]').forEach((templateEl) => {
        const memberId = templateEl.getAttribute('data-member-edit-template') || '';
        if (memberId) {
          templateMap.set(memberId, templateEl);
        }
      });
      ...
      const openMemberEdit = (memberId) => {
        ...
        const templateEl = templateMap.get(key);
        const modalTitle = templateEl?.getAttribute('data-member-edit-template-title') || 'Edit Jemaat';
        const templateHtml = templateEl ? templateEl.innerHTML : '';
        ...
        memberEditModal.classList.add('is-open');
      ...
      const closeMemberEdit = () => {
        memberEditModal.classList.remove('is-open');
        memberEditModal.setAttribute('aria-hidden', 'true');
        document.body.classList.remove('modal-open');
      };

      document.addEventListener('click', function (event) {
        const trigger = event.target.closest('[data-member-edit-open]');
        if (!trigger) {
          return;
        }
        event.preventDefault();
        openMemberEdit(trigger.getAttribute('data-member-edit-open') || '');
      });

      memberEditModal.addEventListener('click', function (event) {
        if (event.target === memberEditModal) {
          closeMemberEdit();
          return;
        }
        const closeTrigger = event.target.closest('[data-member-edit-close]');
        if (closeTrigger) {
          event.preventDefault();
          closeMemberEdit();
        }
      ...
      document.addEventListener('keydown', function (event) {
        if (event.key === 'Escape' && memberEditModal.classList.contains('is-open')) {
          closeMemberEdit();
        }
      });

      const autoOpenId = memberEditModal.getAttribute('data-member-edit-auto-open') || '';
      if (autoOpenId) {
        openMemberEdit(autoOpenId);
      }
    }

    const memberCreateModal = document.querySelector('[data-member-create-modal]');
    if (memberCreateModal) {
      const openMemberCreate = () => {
        memberCreateModal.classList.add('is-open');
        memberCreateModal.setAttribute('aria-hidden', 'false');
        document.body.classList.add('modal-open');

        const firstInput = memberCreateModal.querySelector('input, select, textarea');
        if (firstInput && typeof firstInput.focus === 'function') {
          firstInput.focus();
        }
      };

      const closeMemberCreate = () => {
        memberCreateModal.classList.remove('is-open');
        memberCreateModal.setAttribute('aria-hidden', 'true');
        document.body.classList.remove('modal-open');
      };

      document.addEventListener('click', function (event) {
        const trigger = event.target.closest('[data-member-create-open]');
        if (!trigger) {
          return;
        }
        event.preventDefault();
      ...
      memberCreateModal.addEventListener('click', function (event) {
        if (event.target === memberCreateModal) {
          closeMemberCreate();
          return;
        }
        const closeTrigger = event.target.closest('[data-member-create-close]');
        if (closeTrigger) {
          event.preventDefault();
          closeMemberCreate();
        }
      ...
      document.addEventListener('keydown', function (event) {
        if (event.key === 'Escape' && memberCreateModal.classList.contains('is-open')) {
          closeMemberCreate();
        }
      });
    }
