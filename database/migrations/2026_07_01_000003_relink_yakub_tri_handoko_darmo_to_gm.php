...
{
    private const DARMO_BRANCH_ID = 3;

    private const GM_BRANCH_ID = 2;

    private const DARMO_EXTERNAL_PERSON_ID = 776;
    ...
    public function up(): void
    {
        ...
        $hasExternal = DB::table('discipleship_people')
            ->where('id', self::DARMO_EXTERNAL_PERSON_ID)
            ->where('branch_id', self::DARMO_BRANCH_ID)
            ->exists();
    ...
    private function relinkGroupLeaderships(): void
    {
        ...
        DB::table('discipleship_group_people')
            ->where('branch_id', self::DARMO_BRANCH_ID)
            ->where('person_id', self::DARMO_EXTERNAL_PERSON_ID)
            ->where('role', '!=', 'member')
    ...
    private function relinkMentorships(): void
    {
        ...
        DB::table('discipleship_relationships')
            ->where('branch_id', self::DARMO_BRANCH_ID)
            ->where('mentor_person_id', self::DARMO_EXTERNAL_PERSON_ID)
    ...
    private function relinkGroupInitiators(): void
    {
        ...
        DB::table('discipleship_groups')
            ->where('branch_id', self::DARMO_BRANCH_ID)
            ->where('initiated_by_person_id', self::DARMO_EXTERNAL_PERSON_ID)
    ...
    private function relinkReports(): void
    {
        ...
        DB::table('discipleship_meeting_reports')
            ->where('branch_id', self::DARMO_BRANCH_ID)
            ->where('leader_person_id', self::DARMO_EXTERNAL_PERSON_ID)
    ...
    private function relinkFeedbacks(): void
    {
        ...
        DB::table('discipleship_feedbacks')
            ->where('branch_id', self::DARMO_BRANCH_ID)
            ->where('respondent_person_id', self::DARMO_EXTERNAL_PERSON_ID)
            ->update($this->valuesWithTimestamp('discipleship_feedbacks', [
                'respondent_person_id' => self::GM_PERSON_ID,
            ]));
    }
    ...
    private function closeExternalRootRelations(): void
    {
        ...
        DB::table('discipleship_relationships')
            ->where('branch_id', self::DARMO_BRANCH_ID)
            ->where('disciple_person_id', self::DARMO_EXTERNAL_PERSON_ID)
            ->where('status', 'active')
    ...
    private function invalidateDiscipleshipReadCache(): void
    {
        ...
        foreach ([self::DARMO_BRANCH_ID, self::GM_BRANCH_ID] as $branchId) {
