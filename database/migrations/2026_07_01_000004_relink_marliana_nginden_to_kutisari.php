...
return new class extends Migration
{
    ...
    private const NGINDEN_BRANCH_ID = 6;

    private const KUTISARI_PERSON_ID = 664;

    private const NGINDEN_EXTERNAL_PERSON_ID = 854;
    ...
    public function up(): void
    {
        ...
        $hasExternal = DB::table('discipleship_people')
            ->where('id', self::NGINDEN_EXTERNAL_PERSON_ID)
            ->where('branch_id', self::NGINDEN_BRANCH_ID)
            ->exists();
        $hasCanonical = DB::table('discipleship_people')
            ->where('id', self::KUTISARI_PERSON_ID)
    ...
    {
        // Intentionally irreversible: this migration preserves the canonical Kutisari person link.
    }
    ...
    private function relinkGroupLeaderships(): void
    {
        ...
        DB::table('discipleship_group_people')
            ->where('branch_id', self::NGINDEN_BRANCH_ID)
            ->where('person_id', self::NGINDEN_EXTERNAL_PERSON_ID)
            ->where('role', '!=', 'member')
            ->update($this->valuesWithTimestamp('discipleship_group_people', [
                'person_id' => self::KUTISARI_PERSON_ID,
            ]));
    }
    ...
    private function relinkMentorships(): void
    {
        ...
        DB::table('discipleship_relationships')
            ->where('branch_id', self::NGINDEN_BRANCH_ID)
            ->where('mentor_person_id', self::NGINDEN_EXTERNAL_PERSON_ID)
            ->update($this->valuesWithTimestamp('discipleship_relationships', [
                'mentor_person_id' => self::KUTISARI_PERSON_ID,
            ]));
    }
    ...
    private function relinkGroupInitiators(): void
    {
        ...
        DB::table('discipleship_groups')
            ->where('branch_id', self::NGINDEN_BRANCH_ID)
            ->where('initiated_by_person_id', self::NGINDEN_EXTERNAL_PERSON_ID)
            ->update($this->valuesWithTimestamp('discipleship_groups', [
                'initiated_by_person_id' => self::KUTISARI_PERSON_ID,
            ]));
    ...
    private function relinkReports(): void
    {
        ...
        DB::table('discipleship_meeting_reports')
            ->where('branch_id', self::NGINDEN_BRANCH_ID)
            ->where('leader_person_id', self::NGINDEN_EXTERNAL_PERSON_ID)
            ->update($this->valuesWithTimestamp('discipleship_meeting_reports', [
                'leader_person_id' => self::KUTISARI_PERSON_ID,
            ]));
    }
    ...
    private function relinkFeedbacks(): void
    {
        ...
        DB::table('discipleship_feedbacks')
            ->where('branch_id', self::NGINDEN_BRANCH_ID)
            ->where('respondent_person_id', self::NGINDEN_EXTERNAL_PERSON_ID)
            ->update($this->valuesWithTimestamp('discipleship_feedbacks', [
                'respondent_person_id' => self::KUTISARI_PERSON_ID,
            ]));
    }
    ...
    private function closeExternalRootRelations(): void
    {
        ...
        DB::table('discipleship_relationships')
            ->where('branch_id', self::NGINDEN_BRANCH_ID)
            ->where('disciple_person_id', self::NGINDEN_EXTERNAL_PERSON_ID)
            ->where('status', 'active')
    ...
    private function invalidateDiscipleshipReadCache(): void
    {
        ...
        foreach ([self::KUTISARI_BRANCH_ID, self::NGINDEN_BRANCH_ID] as $branchId) {
